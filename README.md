...
<div align="center">
  ...
  <p><b>让 AI 在 Android 设备上调用工具、操作环境并完成任务</b></p>
</div>
...
**Operit AI** 是一款运行在 Android 设备上的可扩展 AI Agent 应用。它将模型对话与文件、终端、浏览器、应用界面、项目工作区和自动化能力连接起来，让 AI 不只回答问题，还能在设备上执行实际任务。

聊天记录和模型配置保存在设备本地。您可以自行接入云模型服务，也可以通过 MNN 或 llama.cpp 在设备上运行本地模型。脚本、ToolPkg、MCP、Skill 和工作流共同组成可扩展的工具生态。
...
- **本地数据**：聊天记录、角色、记忆和模型配置由应用保存在设备存储中；应用内备份、Android 系统备份和用户主动导出可能复制这些数据
- **云模型**：内置云模型连接由设备直接请求用户配置的服务商端点；Operit 不提供托管的聊天推理服务，也不中转这些聊天请求
- **本地模型**：MNN 和 llama.cpp 在设备上执行推理；准备好模型文件后，可在不连接云模型服务商的情况下使用
- **其他网络功能**：市场、公告、更新检查、GitHub 登录，以及用户启用的 MCP、搜索、语音和绘图服务会访问相应网络资源
- **第三方扩展**：脚本、ToolPkg、MCP 和 Skill 可能连接各自声明的外部服务，安装和授权前请查看其来源与说明
- **对外服务**：Web Chat 和 HTTP API 默认关闭；启用后会监听设备网络接口，聊天与 API 请求要求 Bearer Token，资源链接采用独立访问方式，请仅在受信网络中向已授权用户开放
- **本机应用集成**：Android Intent 和广播接口独立于 HTTP 开关与 Token，用于 Tasker 等本机自动化，请仅与受信应用配合使用
...
本项目主许可证为 [GNU LGPLv3](LICENSE)。仓库包含的第三方组件可能采用各自的许可证，具体条款以对应许可证文件为准。
